...
function buildPrompt(styleRaw: string, roomTypeRaw: string) {
  ...
  if (!isExterior) {
    const interiorModifiers: Record<string, string> = {
      ...
      "Scandinavian": "ultra-refined Scandinavian and contemporary minimal luxury interior, bathed in soft diffused natural daylight from the existing windows and warm recessed cove ceiling lighting. PRESERVE ALL EXISTING WINDOWS, GLASS DOORS AND ARCHITECTURAL OPENINGS EXACTLY — do not block, cover or alter them in any way. The centrepiece is a grand fully upholstered luxury platform bed with a tall generously padded bouclé or fine linen headboard in warm ivory (NOT raw wood, NOT a wooden headboard), on a solid low-profile upholstered base in soft oat or warm greige. The bed is dressed in multiple layers of the finest premium bedding: crisp white Egyptian cotton sheets, a plush waffle-knit duvet cover in warm white, at least five oversized pillows in white and ivory in mixed textures (linen, bouclé, and cotton), and a chunky-knit throw in warm beige casually folded at the foot. Two matching minimal ceramic or plaster-finish bedside nightstands flank the bed, each topped with a textured rustic ceramic table lamp with a linen shade, a small stack of books, and a single ceramic mug. A grand rustic heavily textured organic ceramic floor vase holds a massive magnificent arrangement of fluffy pampas grass plumes — this vase is mandatory and must always be present. A large mature olive tree in a textured terracotta pot stands in a corner away from any window or door. A cozy oversized cream bouclé teddy-style luxury lounge armchair with rounded arms sits in a free corner with a soft knit throw casually draped over one arm, positioned next to a small organically shaped solid oak side stool with a ceramic tray on top. A single elegant minimalist line-art abstract drawing in a thin natural wood frame hangs on the solid wall above or behind the bed. A minimal full-length mirror leans against a solid wall only. A small slim built-in wardrobe unit with light oak finish is placed only on a solid wall that has no windows or doors — it must not obstruct or cover any glass opening. A textured natural chunky-weave area rug in warm oat covers the light wide-plank oak floor. Sheer premium linen curtains in warm ivory are drawn softly over the windows",
