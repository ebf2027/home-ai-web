...
function buildPrompt(styleRaw: string, roomTypeRaw: string) {
  ...
  if (!isExterior) {
    const interiorModifiers: Record<string, string> = {
      ...
      "Scandinavian": "wide-plank light oak flooring throughout, white matte plaster walls with all original windows and doors preserved in exact positions and sizes, floor-length natural sand linen curtains framing all windows, Nordic minimalist furniture with clean lines in light oak wood and soft ivory upholstery, neutral monochromatic palette (white, cream, beige, light grey, natural wood), layered natural textiles (linen, wool, cotton) in oatmeal and ivory, large natural fiber area rugs, potted indoor greenery in terracotta planters, simple botanical line-art in light oak frames, warm hygge atmosphere with functional Scandinavian design",
