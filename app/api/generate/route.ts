...
function buildPrompt(styleRaw: string, roomTypeRaw: string) {
  ...
  if (!isExterior) {
    const interiorModifiers: Record<string, string> = {
      ...
      "Super Luxury": "bespoke book-matched Calacatta marble feature wall behind bed with gold veining and polished high-gloss finish, custom Italian boiserie wall paneling with fluted details and brushed champagne gold or brass inlay hardware, hand-stitched deep channel-tufted velvet upholstered headboard in jewel tones (emerald, sapphire, or champagne), grand multi-tier crystal chandelier with hundreds of faceted prisms or dramatic oversized sculptural art-light installation, statement oversized designer furniture (curved velvet sofa, sculptural lounge chair, mirrored nightstands), herringbone or chevron white oak floors with satin finish, silk curtains with cascading drapes and hidden motorized tracks, fresh orchid arrangements in crystal vases, museum-quality original oil paintings or large-scale photography in ornate gold frames, integrated smart lighting with color-changing LED strips, floating fireplace with backlit onyx surround",
    };
