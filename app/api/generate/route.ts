...
function buildPrompt(styleRaw: string, roomTypeRaw: string) {
  ...
  if (!isExterior) {
    const interiorModifiers: Record<string, string> = {
      ...
      "Scandinavian": "preserve all existing windows and doors in exact positions, wide-plank light oak flooring, white matte plaster walls, natural light wood window frames with floor-length sand linen curtains, Nordic minimalist furniture in light oak with soft ivory upholstery, neutral palette (white, cream, beige, natural wood), layered natural textiles (linen, wool, cotton) in oatmeal tones, natural fiber area rugs, potted greenery in terracotta planters, botanical line-art in light wood frames",
...
async function callFalImageEdit(args: {
  ...
  const { imageFile, prompt, guidanceScale = 13 } = args;
