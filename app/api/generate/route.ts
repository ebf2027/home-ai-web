...
function buildPrompt(styleRaw: string, roomTypeRaw: string) {
  ...
  if (!isExterior) {
    const interiorModifiers: Record<string, string> = {
      ...
      "Scandinavian": "wide-plank light oak floors, white matte plaster walls, tall padded ivory linen headboard against the wall, white linen bedding with chunky oatmeal knit throw, solid oak nightstands with ceramic lamps, large oatmeal wool area rug under the bed, curved bouclé ivory armchair with oak legs in corner, open oak shelving unit against the wall, tall olive tree in terracotta pot, large botanical line-art print in oak frame, floor-length sand linen curtains framing the existing windows",
