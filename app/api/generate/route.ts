...
function buildPrompt(styleRaw: string, roomTypeRaw: string) {
  ...
  if (!isExterior) {
    const interiorModifiers: Record<string, string> = {
      ...
      "Scandinavian": "ultra-refined Scandinavian and contemporary minimal luxury interior, bathed in soft, diffused natural daylight from grand expansive floor-to-ceiling black-framed Crittall-style windows and warm recessed cove ceiling lighting, featuring a fully upholstered (fine linen or bouclé) platform bed with a soft padded headboard dressed in layered premium white and ivory linen, oversized pillows, and a textured waffle-knit beige coverlet layered with a soft knit throw. Simple minimal light oak nightstands hold a textured rustic ceramic lamp with a linen shade, books, and a single ceramic mug. A grand rustic heavily textured organic ceramic floor vase holds a massive magnificent arrangement of fluffy pampas grass plumes. A large mature olive tree in a textured terracotta pot is present. A cozy cream bouclé (teddy-style) lounge chair has a soft knit throw draped over it, positioned next to an organically shaped solid wood stool (like a 'Stool'). A single elegant minimalist line-art abstract drawing in a natural wood frame hangs above the bed. A minimal full-length mirror is leaning against the wall nearby. A built-in custom light wood wardrobe unit with open shelves and drawers. A textured area rug is on the light wood floor. Sheer linen curtains are drawn over the windows",
